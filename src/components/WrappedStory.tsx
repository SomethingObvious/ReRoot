...
export default function WrappedStory({ onClose }: Props) {
  ...
  const handleShare = () => {
    ...
    flash.style.cssText =
      "position:fixed;inset:0;background:white;z-index:9999;pointer-events:none;opacity:1;transition:opacity 0.4s";
          ...
          <motion.button
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.backgroundImage =
                "linear-gradient(rgba(255,255,255,0.15), rgba(255,255,255,0.15)), linear-gradient(135deg, #E1306C, #F77737, #FCAF45)";
...
function Slide0() {
  return (
    ...
    >
      <motion.div custom={0} variants={textFadeUp} className="text-7xl mb-4">
        🐷
      </motion.div>
      <motion.p
        custom={1}
        variants={textFadeUp}
        className="text-sm font-outfit font-semibold text-white/60 uppercase tracking-widest mb-2"
      >
      ...
      <motion.p custom={3} variants={textFadeUp} className="text-lg font-outfit text-white/80 mb-6">
        That's <span className="font-bold text-white">14 coffees</span> ☕ or{" "}
        <span className="font-bold text-white">2 months of Netflix</span>
      </motion.p>
      <motion.div
        custom={4}
        variants={textFadeUp}
    ...
    >
      ...
      <motion.div custom={2} variants={textFadeUp} className="w-full max-w-[280px] h-[200px] mb-4">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={radarData} cx="50%" cy="50%">
            ...
            <PolarAngleAxis
              dataKey="stat"
              tick={{ fill: "rgba(255,255,255,0.7)", fontSize: 11, fontFamily: "Outfit" }}
            />
      ...
      >
        <p className="text-sm font-outfit text-white/80">
          You're a <span className="font-bold text-emerald-300">Fiber Fanatic</span>. Your veggie intake is in the{" "}
  ...
  return (
    ...
    >
      <motion.p
        custom={0}
        variants={textFadeUp}
        className="text-sm font-outfit font-semibold text-white/60 uppercase tracking-widest mb-3"
      >
        Your Eco Impact
      </motion.p>
  ...
  return (
    ...
    >
      ...
      <motion.div
        custom={3}
        variants={textFadeUp}
        className="rounded-2xl bg-white/10 backdrop-blur p-4 border border-white/15 mb-2"
      >
        <div className="flex gap-1.5 mb-2">
          {heatmapDays.map((d) => (
            <span key={d} className="text-[9px] font-outfit text-white/50 w-8 text-center">
              {d}
            </span>
          ))}
        </div>
  ...
  return (
    ...
    >
      ...
      <motion.p
        custom={0}
        variants={textFadeUp}
        className="text-sm font-outfit font-semibold text-white/60 uppercase tracking-widest mb-3 relative z-10"
      >
        The Moment of Truth
      </motion.p>
      <motion.h1
        custom={1}
        variants={textFadeUp}
        className="text-4xl font-outfit font-extrabold text-white mb-2 relative z-10"
      >
        Gouda Vibes Only 🧀
      </motion.h1>
